...
| nux | `fix-missing-vidmode.patch` (0ubuntu13+unity1) | `CreateOpenGLWindow` dereferenced an unchecked XF86VidMode mode list (`GraphicsDisplayX11.cpp:297`): any standalone Nux window segfaulted on Xvfb and Xvnc, which kept Unity's tests off Xvfb. Unity itself is not affected. Ours; built, verified on target2, awaiting agent A's Unity test run before aptly. | none (upstream on hold) | [`research/nux-vidmode/`](research/nux-vidmode/) | draft |
