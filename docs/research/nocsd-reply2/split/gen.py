...
def pick(text):
    ...
    for line in text.split('\n'):
        m = re.match(r'\s*//@@(!?)([ABC])$', line)
        e = re.match(r'\s*//@@/(!?)([ABC])$', line)
        if m:
...
{f('gtk_widget_action_set_enabled', 'if (!GTKNoCSDMenuReady ||', 'if (!GTKNoCSDMenuProxyReady ||')}//@@/A
