...
`c0557df`), 0.6 (gjs/Python through `g_module_symbol`, `0ce1eb1`), all built
with sbuild and published to aptly. Nothing in flight.

Proposed next, none started - May decides: menus built after realize
(font-viewer, contacts); enabled state for stand-ins (interposing
`gtk_widget_action_set_enabled`), which would also fix `hidden-when` pairs;
whether to report the two gtk-nocsd findings upstream.

## State of `target2`

Dirty (`~/.dirty`): `libunity-gtk4-menu0` 0.6 installed with `dpkg -i`
...
`~/b/crash-0.6/` - all accounted for in `research/layer-b/`.
